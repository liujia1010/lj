<?php
// 多继承实现方式：使用 trait 关键字定义父类，在子类中使用  use 关键字引入,多继承会产生继承冲突，如果不同的类中有同名方法，就需要指定类的优先级,示例如下

// 优先级关系：从基类继承的成员会被trait插入的成员覆盖，当前类的成员可以覆盖trait类的成员，同名方法调用优先级—— 当前类成员> trait插入的成员> 基类成员
// 
// 如果两个 trait 类中存在同名方法，会出现Trait method save has not been applied, because there are collisions with other trait methods on Topi错误，两个trait类中出现同名方法，就需要处理同名方法
// 
/**
 * 通用类Common
 */
...
class Topic extends Common{
	...
	use Log,Comment{
		...
		Comment::save as send ;// 如果想要继续使用Comment中的save方法，使用as 关键字，重命名
	}
}
